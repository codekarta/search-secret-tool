...
function isExcludedFile(fileName, fullPath = '') {
  ...
  if (typeof GLOBAL_ALLOWLIST !== 'undefined' && GLOBAL_ALLOWLIST.paths && GLOBAL_ALLOWLIST.paths.length > 0) {
    if (GLOBAL_ALLOWLIST.paths.some(rx => { rx.lastIndex = 0; return rx.test(fileName) || (fullPath && rx.test(fullPath)); })) {
      return true;
    }
  }
...
function cleanGoRegex(rawPattern) {
  ...
  // Handle (?i:...) and (?-i:...) inline flag modifiers -> preserve (?:...) non-capturing group
  p = p.replace(/\(\?i:/g, '(?:');
  p = p.replace(/\(\?-i:/g, '(?:');
...
/**
 * Robustly extracts array of string values for a given key from a TOML block.
 * Correctly avoids splitting at character classes (e.g. `[a-z]`) inside patterns.
 */
function extractTomlArray(block, key) {
  const rx = new RegExp(`^\\s*${key}\\s*=\\s*\\[([\\s\\S]*?)\\n\\s*\\]`, 'm');
  ...
  for (const line of lines) {
    ...
    if (itemMatch) {
      const val = itemMatch[1] ?? itemMatch[2] ?? itemMatch[3] ?? itemMatch[4];
...
function parseGitleaksToml(content, fileName = 'gitleaks.toml') {
  ...
  for (let i = 1; i < ruleBlocks.length; i++) {
    ...
    const allowlistBlocks = block.split(/\[\[rules\.allowlists\]\]|\[rules\.allowlists\]/);
    const mainRulePart = allowlistBlocks[0];

    const idMatch = mainRulePart.match(/^\s*id\s*=\s*["']([^"']+)["']/m);
    const descMatch = mainRulePart.match(/^\s*description\s*=\s*["']([^"']+)["']/m);
    const entropyMatch = mainRulePart.match(/^\s*entropy\s*=\s*([0-9.]+)/m);
    const secretGroupMatch = mainRulePart.match(/^\s*secretGroup\s*=\s*(\d+)/m);

    // Regex pattern (supports ''', """, ', or ")
    let regexStr = null;
    const rawRegexMatch = mainRulePart.match(/^\s*regex\s*=\s*(?:'''([\s\S]*?)'''|"""([\s\S]*?)"""|'([^'\r\n]*)'|"([^"\r\n]*)")/m);
    if (rawRegexMatch) {
      regexStr = rawRegexMatch[1] || rawRegexMatch[2] || rawRegexMatch[3] || rawRegexMatch[4] || null;
    }

    // Path pattern filter (optional)
    const pathMatch = mainRulePart.match(/^\s*path\s*=\s*(?:'''([\s\S]*?)'''|"""([\s\S]*?)"""|'([^'\r\n]*)'|"([^"\r\n]*)")/m);

    // Keywords pre-filter (optional)
    const rawKeywords = extractTomlArray(mainRulePart, 'keywords');
    const keywords = rawKeywords.map(k => k.toLowerCase());

    // Parse [[rules.allowlists]] attached to this rule
    const allowlists = [];
    for (let j = 1; j < allowlistBlocks.length; j++) {
      const alBlock = allowlistBlocks[j];
      const targetMatch = alBlock.match(/^\s*regexTarget\s*=\s*["']([^"']+)["']/m);
      const regexTarget = targetMatch ? targetMatch[1].trim().toLowerCase() : 'secret';

      // Parse regexes
      const rawRegexes = extractTomlArray(alBlock, 'regexes');
      const compiledRegexes = [];
      for (const raw of rawRegexes) {
        try {
          const { pattern, flags } = cleanGoRegex(raw);
          compiledRegexes.push(new RegExp(pattern, flags));
        } catch (e) {}
      }

      // Parse stopwords
      const stopwords = extractTomlArray(alBlock, 'stopwords').map(s => s.toLowerCase());

      // Parse paths
      const rawPaths = extractTomlArray(alBlock, 'paths');
      const compiledPaths = [];
      for (const p of rawPaths) {
        try {
          const { pattern, flags } = cleanGoRegex(p);
          compiledPaths.push(new RegExp(pattern, flags.replace('g', '')));
        } catch (e) {}
      }

      if (compiledRegexes.length > 0 || stopwords.length > 0 || compiledPaths.length > 0) {
        allowlists.push({
          regexTarget,
    ...
    if (idMatch && regexStr) {
      ...
      let secretGroup = secretGroupMatch ? parseInt(secretGroupMatch[1], 10) : null;
      const entropy = entropyMatch ? parseFloat(entropyMatch[1]) : null;

      try {
        const { pattern, flags } = cleanGoRegex(regexStr.trim());
        const compiledRegex = new RegExp(pattern, flags);

        // If secretGroup is not explicitly specified, default to group 1 if regex contains capture groups
        if (secretGroup === null && /(?<!\\)\((?!\?)/.test(pattern)) {
          secretGroup = 1;
        }
...
/**
 * Parses global [allowlist] / [[allowlists]] defined in Gitleaks TOML files.
 */
function parseGlobalAllowlists(content) {
  const globalAllowlist = { paths: [], regexes: [], stopwords: [] };
  const headerBlock = content.split(/\[\[rules\]\]/)[0] || '';
  if (!headerBlock.includes('[allowlist]') && !headerBlock.includes('[[allowlists]]')) {
    return globalAllowlist;
  }

  // Parse global paths
  const rawPaths = extractTomlArray(headerBlock, 'paths');
  for (const p of rawPaths) {
    try {
      const { pattern, flags } = cleanGoRegex(p);
      globalAllowlist.paths.push(new RegExp(pattern, flags.replace('g', '')));
    } catch (e) {}
  }

  // Parse global regexes
  const rawRegexes = extractTomlArray(headerBlock, 'regexes');
  for (const r of rawRegexes) {
    try {
      const { pattern, flags } = cleanGoRegex(r);
      globalAllowlist.regexes.push(new RegExp(pattern, flags));
    } catch (e) {}
  }

  // Parse global stopwords
  globalAllowlist.stopwords = extractTomlArray(headerBlock, 'stopwords').map(s => s.toLowerCase());
...
function discoverAndLoadTomlRules(rootDir) {
    ...
    if (fs.existsSync(rootDir)) {
      ...
      for (const tomlFile of tomlFiles) {
        ...
        try {
          ...
          const parsedGlobalAl = parseGlobalAllowlists(content);
          globalAllowlist.paths.push(...parsedGlobalAl.paths);
          globalAllowlist.regexes.push(...parsedGlobalAl.regexes);
          globalAllowlist.stopwords.push(...parsedGlobalAl.stopwords);
  ...
  return { rules: loadedRules, summaries: tomlSummaries, fileNames: tomlFileNames, globalAllowlist };
}

// Discover and initialize rules from all *.toml files in the project root
const { rules: TOML_RULES, summaries: TOML_RULE_SUMMARIES, fileNames: TOML_RULE_FILES, globalAllowlist: GLOBAL_ALLOWLIST } = discoverAndLoadTomlRules(__dirname);
 ...
 * e.g. "devClientSecret" -> ["dev", "client", "secret"]
 */
 ...
 *   public static final String USER_KEY = "user.key";
 *   public static final String USER_KEY = "soemthing-user-somethingelse-key";
 *   public static final String DATA_CATALOG_CLIENT_SECRET_DEV = "devClientSecret";
 *   const API_KEY_HEADER = "x-api-key";
...
function isConfigKeyIdentifier(rawMatch, lineText = '', fullFileContent = '') {
  ...
  // -------------------------------------------------------------
  // 3. Exact, Substring, and Token Overlap Match with Variable Name
  // -------------------------------------------------------------
 ...
 * Real secrets/API keys generally have higher entropy (>= 3.0-3.5)
 * than natural language words or variable names.
 */
...
/**
 * Checks if a matched value in source code is simply an unquoted code identifier / parameter / variable name
 * rather than a secret literal (e.g. dataCatalogRestApiConfigVo, dataLeadDataOfficeLobResultVO, myVar).
 */
function isCodeVariableOrSymbol(rawMatch, matchedSecretValue, lineText = '') {
  if (!matchedSecretValue) return false;
  const cleanVal = matchedSecretValue.trim().replace(/^["'`]|["'`]$/g, '');
  if (!cleanVal) return false;

  // Safeguard: Never suppress known high-confidence token signatures
  if (/^(?:eyJ|AKIA|ghp_|gho_|ghu_|ghs_|ghr_|github_pat_|sk-|AIza|\$2[aby]\$)/.test(cleanVal)) {
    return false;
  }

  // Safeguard: Real passwords with symbols (@, !, #, $, %, etc.)
  if (/[@!#$%^&*+=~?><]/.test(cleanVal)) {
    return false;
  }

  // If the secret value is a single bare identifier (e.g. camelCase / PascalCase word)
  const isBareIdentifier = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(cleanVal);
  if (!isBareIdentifier) return false;

  // Check if cleanVal appears enclosed in quotes inside lineText
  const isQuoted = new RegExp(`["'\`]${cleanVal.replace(/[$]/g, '\\$')}["'\`]`).test(lineText);
  if (isQuoted) return false; // Quoted string literal like "hello123" -> not an unquoted variable

  // Unquoted single code identifier (e.g., parameter, type, or variable reference in code)
  return true;
}
...
function scanSingleFile(fullPath, rootDir, findings, stats) {
  ...
  for (const pattern of ALL_PATTERNS) {
    ...
    while ((match = pattern.regex.exec(scanContent)) !== null) {
      ...
      if (typeof GLOBAL_ALLOWLIST !== 'undefined' && GLOBAL_ALLOWLIST) {
        if (GLOBAL_ALLOWLIST.regexes && GLOBAL_ALLOWLIST.regexes.some(rx => { rx.lastIndex = 0; return rx.test(rawMatch) || rx.test(matchedSecretValue); })) {
          if (pattern.regex.lastIndex === matchIndex) pattern.regex.lastIndex++;
          continue;
        }
        if (GLOBAL_ALLOWLIST.stopwords && GLOBAL_ALLOWLIST.stopwords.includes(matchedSecretValue.toLowerCase().trim())) {
          if (pattern.regex.lastIndex === matchIndex) pattern.regex.lastIndex++;
          continue;
        }
      }

      // Entropy filter: Skip matches if Shannon entropy is below rule threshold (e.g. Gitleaks entropy = 3.5)
      if (typeof pattern.entropy === 'number' && pattern.entropy > 0) {
        const valEntropy = calculateShannonEntropy(matchedSecretValue);
        if (valEntropy < pattern.entropy) {
          if (pattern.regex.lastIndex === matchIndex) pattern.regex.lastIndex++;
          continue;
        }
      }

      // Rule allowlists (from Gitleaks [[rules.allowlists]])
      if (pattern.allowlists && pattern.allowlists.length > 0) {
        let isAllowlisted = false;
        for (const al of pattern.allowlists) {
          const targetText = al.regexTarget === 'match' ? rawMatch : matchedSecretValue;
          if (al.regexes && al.regexes.some(rx => { rx.lastIndex = 0; return rx.test(targetText); })) {
            isAllowlisted = true;
            break;
          }
          if (al.stopwords && al.stopwords.includes(matchedSecretValue.toLowerCase().trim())) {
            isAllowlisted = true;
            break;
          }
          if (al.paths && al.paths.some(rx => { rx.lastIndex = 0; return rx.test(relPath) || rx.test(fullPath); })) {
            isAllowlisted = true;
            break;
          }
        }
      ...
      // FP-5: Skip unquoted code variables, method arguments, and class/type symbols
      if (isCodeVariableOrSymbol(rawMatch, matchedSecretValue, matchingLine)) {
        if (pattern.regex.lastIndex === matchIndex) pattern.regex.lastIndex++;
        continue;
      }
